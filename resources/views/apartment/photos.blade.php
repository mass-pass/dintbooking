...
<title>List Your Property - photos</title>
	@section('main')
					...
					<div class="main-panel min-height mt-4">
						<div class="row justify-content-center">
							...
							<div class="col-md-9 mt-4 mt-sm-0 pl-4 pr-4">
									...
									<div class="row">
										<div class="col-md-10 border mt-4 pb-5 rounded-3 pl-sm-0 pr-sm-0 ">
											<div class="row">
												...
												<p class="mt-1"><strong>Add at least one photo.</strong> You can always add more later
                        </p>
												<form  action="{{url('listing/'.$result->id.'/list-upload')}}" class="dropzone" id="DropzoneElement">{{ csrf_field() }}</form>



												<form id="img_form" enctype='multipart/form-data' method="post" action="{{url('listing/'.$result->id.'/'.'photo-upload')}}" accept-charset='UTF-8'>
									             {{ csrf_field() }}
									...
									<div class="col-md-12 p-0 mt-4 mb-5">
										<div class="row m-0 justify-content-between">
											...
											<div class="mt-4">
												...
												<a href="{{url('list/'.$result->id.'/credit-card')}}" class=" btn thme-btn w-100 btn vbtn-outline-success text-16 font-weight-700 pl-5 pr-5 pt-3 pb-3 pl-5 pr-5" id="btnnext"><i class="spinner fa fa-spinner fa-spin d-none" id="btn_spin"></i>
													<span id="btnnext-text">Continue</span>
												</a>
											</div>
										</div>  
									</div>
...
Dropzone.autoDiscover = false;
    $(function(){
      ...
      var myDropzone = new Dropzone("form#DropzoneElement", {  url: "{{ url('listing/'.$result->id.'/photo-upload') }}" });
      console.log({dd:myDropzone});
      myDropzone.on("queuecomplete", function(file, res) {
        $.ajax({
          url: '/listing/<?php echo $result->id;?>/photo-selectables',
