...
window.customDaterangeFormat = function () {
...
window.dateRangeBtn = function (startDate, endDate, dt = null) {
	var df = dt;
...
window.formDate = function (startDate, endDate) {
	var customFormat = customDaterangeFormat();
	var init = moment(0);
	var initdate;
...
window.bookingSegments = {
	increase: function (a) {
		let span = $(a).prev('span.value-room');
		let value = parseInt(span.text());
		span.text(++value);
		let target = $(a).data('target');
		$("#hdn" + target).val(value);
		$(".spn" + target).text(value + " " + target.toLowerCase());
		if (value > 0) $(a).parent().find('a').removeClass('desbled');
	},
	decrease: function (a) {
		let span = $(a).next('span.value-room');
		let value = parseInt(span.text());
		if (value == 0)
			return;
		...
		$("#hdn" + target).val(value);
		$(".spn" + target).text(value + " " + target.toLowerCase());
		if (value == 0) $(a).addClass('desbled');
	},
	setTraveling: function (isTraveling) {
		$("#traveling").val(0);
		if (isTraveling) $("#traveling").val(1);
	},
}