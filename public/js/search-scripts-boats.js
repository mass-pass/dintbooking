(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["/js/search-scripts-boats"],{

/***/ "./resources/js/search-scripts-boats.js":
/*!**********************************************!*\
  !*** ./resources/js/search-scripts-boats.js ***!
  \**********************************************/
...
var buildResult = function buildResult(map, result, req) {
  ...
  for (var key in boats) {
    if (boats.hasOwnProperty(key)) {
      ...
      boat_div += '<div class="col-sm-6 col-md-12 col-lg-12  p-0 mb-4">' + '<div class=" row  border p-2 rounded-3">' + '<div class="col-lg-5 p-2">' + '<div class="img-event">' + '<a href="' + APP_URL + '/boats/' + boats[key].slug + '?travelling_date=' + req.boat_date + '" target="_blank">' + '<img class="img-fluid rounded" src="' + (boats[key].cover_photo ? boats[key].cover_photo : '/images/no-boat-photo.jpeg') + '" alt="' + boats[key].name + '">' + '</a>' + '</div>' + '</div>' + '<div class="col-lg-7 p-2">' + '<div class="row justify-content-between">' + '<div class="col-sm-12 pl-0">' + '<a href="' + APP_URL + '/boats/' + boats[key].slug + '?travelling_date=' + req.boat_date + '" target="_blank">' + '<p class="mb-0 text-18 text-color font-weight-700 text-color-hover text">' + boats[key].name + '</p>' + '</a>' + '</div>' + '</div>' + '<div class="review-0 mt-4">' + '<div class="d-flex justify-content-between">' + '<div>' + '<span><i class="fa fa-star text-14 secondary-text-color"></i>' + ' ' + avg_rating + ' ' + '(' + reviews_count + ')</span>' + '</div>' + '<div>' + '<span class="font-weight-700 text-20">' + symbolWithPrice + '</span> / Per Days' + '</div>' + '</div>' + '</div>' + '<ul class="list-inline mt-2 pb-3">' + '<li class="list-inline-item border rounded-3 p-1 mt-4 pl-3 pr-3">' + '<p class="text-center mb-0">' + '<i class="fas fa-ship text-20 d-none d-sm-inline-block text-muted"></i> ' + boats[key].cabin_count + '<span class=" text-14 font-weight-700"> Cabin</span>' + '</p>' + '</li>' + '<li class="list-inline-item  border rounded-3 mt-4 p-1  pl-3 pr-3">' + '<p  class="text-center mb-0" >' + '<i class="fas fa-user-friends d-none d-sm-inline-block text-20 text-muted"></i> ' + boats[key].authorised_onboard_capacity + '<span class=" text-14 font-weight-700"> OnBoard Capacity</span>' + '</p>' + '</li>' + '<li class="list-inline-item  border rounded-3 mt-4 p-1  pl-3 pr-3">' + '<p  class="text-center mb-0">' + '<i class="fas fa-bath text-20  d-none d-sm-inline-block  text-muted"></i> ' + boats[key].bathroom_count + '<span class="text-14 font-weight-700"> Bathrooms</span>' + '</p>' + '</li>' + '</ul>' + '</div>' + '</div>' + '</div>';
    }
  }
...
function getBoats(map, url) {
  if (loadPage) {
    ...
    var priceRange = $('#price-range').attr('data-value'); //priceRange = priceRange.split(',');
    ...
    $('#minPrice').html(min_price);
    $('#maxPrice').html(max_price);
    var lengthRange = $('#length-range').attr('data-value'); //lengthRange = lengthRange.split(',');
    ...
    $('#minLength').html(min_length);
    $('#maxLength').html(max_length);
    var speedRange = $('#speed-range').attr('data-value'); //speedRange = speedRange.split(',');
    ...
    $('#minSpeed').html(min_speed);
    $('#maxSpeed').html(max_speed);
    var manufactureRange = $('#manufacture-range').attr('data-value'); //manufactureRange = manufactureRange.split(',');
...
$(window).on("load", function () {// allowRefresh = true;
  // loadPage = SearchUrl;
  // getBoats($('#map_view').locationpicker('map').map);
});
...
var searchBoat = new Vue({
  ...
  mounted: function mounted() {
    this.initMap(); // allowRefresh = true;
    // loadPage = SearchUrl;
    // getBoats($('#map_view').locationpicker('map').map);
  },
  methods: {
    initMap: function initMap() {
      $('#map_view').locationpicker(_defineProperty({
        location: {
          latitude: $('#location_lat').val(),
          longitude: $('#location_lng').val()
        },
...
/***/ 2:
/*!****************************************************!*\
  !*** multi ./resources/js/search-scripts-boats.js ***!
  \****************************************************/
/*! no static exports found */
